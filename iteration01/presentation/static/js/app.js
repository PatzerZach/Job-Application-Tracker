...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.alert[data-auto-dismiss]').forEach(el => {
    const delay = Number(el.dataset.autoDismissMs || 4500);
    setTimeout(() => dismissAlert(el), Number.isFinite(delay) ? delay : 4500);
  });
});
...
function initParticles() {
  ...
  class Particle {
    ...
    reset(initial = false) {
      ...
      this.hue = Math.random() > 0.7 ? 200 + Math.random() * 60 : 40 + Math.random() * 15;
    ...
    for (let i = 0; i < particles.length; i++) {
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (dist < MAX_DIST) {
          ...
          ctx.strokeStyle = isDark() ? 'rgba(240,200,74,0.8)' : 'rgba(184,134,11,0.8)';
          ctx.lineWidth = 0.5;
          ctx.beginPath();
...
function init3DTilt() {
  ...
  document.querySelectorAll('.tilt-card, .feature-card, .step-card, .auth-card, .hero-dashboard-inner').forEach(el => {
...
function initMobileSidebar() {
  ...
  const open = () => { sidebar.classList.add('open'); overlay?.classList.add('active'); document.body.style.overflow = 'hidden'; toggle.setAttribute('aria-expanded', 'true'); };
  const close = () => { sidebar.classList.remove('open'); overlay?.classList.remove('active'); document.body.style.overflow = ''; toggle.setAttribute('aria-expanded', 'false'); };
...
function initFileUpload() {
  document.querySelectorAll('.file-upload-area').forEach(area => {
    ...
    ['dragenter', 'dragover'].forEach(evt => area.addEventListener(evt, e => { e.preventDefault(); area.classList.add('dragging'); }));
    ['dragleave', 'drop'].forEach(evt => area.addEventListener(evt, e => { e.preventDefault(); area.classList.remove('dragging'); }));
...
function initDocumentComposers() {
  document.querySelectorAll('[data-document-picker]').forEach(picker => {
    ...
    function validateDocumentPicker() {
      ...
      if (existingSelect) existingSelect.classList.toggle('is-invalid', !valid && mode === 'existing');
      if (fileInput) fileInput.classList.toggle('is-invalid', !valid && mode === 'upload');
      return valid;
    }
    ...
    function syncPanels() {
      ...
      picker.querySelectorAll('[data-document-panel]').forEach(panel => {
        panel.classList.toggle('is-active', panel.dataset.documentPanel === mode);
      });
    ...
    function inferNameFromFile() {
      if (!fileInput || !nameInput || !fileInput.files?.length || nameInput.value.trim()) return;
...
function initConfirmDelete() {
  document.querySelectorAll('[data-confirm]').forEach(btn => {
    btn.addEventListener('click', e => {
      e.preventDefault();
      const message = btn.dataset.confirm || 'Are you sure? This cannot be undone.';
...
function initDocumentPreviewModal() {
  ...
  if (!closeBtn || !openLink || !downloadLink || !titleEl || !subtitleEl || !typeEl || !extensionEl || !loadingEl || !frameShell || !frame || !pdfShell || !pdfStage || !docxShell || !docxStage) {
    return;
  }
  ...
  function showIframePreview(src, currentToken, fallbackMessage) {
    ...
    const timeoutId = window.setTimeout(() => {
      if (didLoad || currentToken !== previewToken || !modal.classList.contains('active')) return;
      showPreviewError(fallbackMessage);
    }, 5000);
    ...
    frameLoadHandler = () => {
      ...
      if (currentToken !== previewToken || !modal.classList.contains('active')) return;
      loadingEl.hidden = true;
      hideAllPreviewPanels();
  ...
  async function renderPdfPreview(contentUrl, currentToken, fallbackMessage) {
    ...
    const pdfDocument = await window.pdfjsLib.getDocument({ data: new Uint8Array(buffer) }).promise;
    if (currentToken !== previewToken || !modal.classList.contains('active')) return;
    ...
    for (let pageNumber = 1; pageNumber <= pdfDocument.numPages; pageNumber += 1) {
      ...
      if (pageNumber === 1 && currentToken === previewToken && modal.classList.contains('active')) {
        loadingEl.hidden = true;
        hideAllPreviewPanels();
        pdfShell.hidden = false;
      }
  ...
  async function loadPreview(button) {
    ...
    if (!endpoint) {
      showPreviewError('Preview details are unavailable for this document right now.');
      return;
    }
    ...
    try {
      ...
      if (currentToken !== previewToken || !modal.classList.contains('active')) return;
      const openUrl = payload.document_open_url || payload.document_external_url || payload.viewer_src || '#';
      const downloadUrl = payload.document_download_url || openUrl;
      const fallbackMessage = payload.viewer_note || 'We could not load the inline preview right now. Try opening the raw file or downloading it.';

      openLink.href = openUrl;
      downloadLink.href = downloadUrl;

      if (payload.viewer_mode === 'pdf' && payload.content_url) {
        await renderPdfPreview(payload.content_url, currentToken, fallbackMessage);
        return;
      }
      ...
      if (payload.viewer_mode === 'docx' && payload.content_url) {
        await renderDocxPreview(payload.content_url, currentToken, fallbackMessage);
        return;
      }
      ...
      if (currentToken !== previewToken || !modal.classList.contains('active')) return;
      showPreviewError('We could not load the inline preview right now. Try opening the raw file or downloading it.');
    }
  }
...
function showConfirmModal(message, onConfirm) {
  ...
  function cleanup() { overlay.classList.remove('active'); confirmBtn.removeEventListener('click', yes); cancelBtn.removeEventListener('click', no); }
  function yes() { cleanup(); onConfirm(); }
  function no()  { cleanup(); }

  confirmBtn.addEventListener('click', yes);
  cancelBtn.addEventListener('click', no);
  overlay.addEventListener('click', e => { if (e.target === overlay) no(); }, { once: true });
}
...
function initAlertDismiss() {
  document.querySelectorAll('.alert-close').forEach(btn => btn.addEventListener('click', () => dismissAlert(btn.closest('.alert'))));
}
...
function initCustomDatePickers() {
  ...
  function renderPicker(picker) {
    ...
    popover.querySelector('[data-date-action="today"]')?.addEventListener('click', () => {
    ...
    popover.querySelector('[data-date-action="clear"]')?.addEventListener('click', () => {
      valueInput.value = '';
  ...
  document.querySelectorAll('[data-number-stepper]').forEach(wrap => {
    ...
    wrap.querySelectorAll('[data-step-action]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        input.value = Number.isFinite(next) ? next.toFixed(step < 1 ? 2 : 0).replace(/\.00$/, '') : '';
...
function initLocationPickers() {
  ...
  function compactLocationLabel(result) {
    return [result.city, result.state, result.country].filter(Boolean).join(', ') || result.label || result.full_label || '';
  }
  ...
  function renderResults(container, results, onSelect) {
    ...
    container.querySelectorAll('[data-location-result-index]').forEach(btn => {
      btn.addEventListener('click', () => onSelect(results[Number(btn.dataset.locationResultIndex)]));
    });
  }
  ...
  pickers.forEach(picker => {
    ...
    async function reverseLookup(lat, lng) {
      try {
        const response = await fetch(`/api/locations/reverse?lat=${encodeURIComponent(lat)}&lng=${encodeURIComponent(lng)}`);
    ...
    if (!enabled || !mapNode || !window.L) {
      if (mapNode) {
        ...
        mapNode.textContent = enabled ? 'Map library is unavailable right now.' : 'Add GEOAPIFY_API_KEY to enable verified search and map selection.';
      }
      return;
    }
    ...
    const currentCountry = form?.querySelector('input[name="job_country"]')?.value;
    const existingLabel = [currentCity, currentState, currentCountry].filter(Boolean).join(', ');
    ...
    resetBtn?.addEventListener('click', () => {
      map.flyTo([defaultView.lat, defaultView.lng], defaultView.zoom, { duration: 0.8 });
    });
        ...
        debounceId = setTimeout(async () => {
          try {
            ...
            if (searchCache.has(cacheKey)) {
              renderResults(resultsWrap, searchCache.get(cacheKey), result => applySelection(result));
              return;
            }
            ...
            renderResults(resultsWrap, results, result => applySelection(result));
          } catch (_) {
            renderResults(resultsWrap, [], () => {});
          }
        }, 450);
    ...
    document.addEventListener('click', () => renderResults(resultsWrap, [], () => {}));
  });
}
...
function isStrictEmail(value) {
  ...
  if (!email || !STRICT_EMAIL_REGEX.test(email)) return false;
...
function getFieldErrorMessage(field) {
  const value = field.type === 'file'
    ? (field.files?.length ? field.files[0].name : '')
    : field.value.trim();
  ...
  if (field.dataset.emailStrict === 'true' && value && !isStrictEmail(value)) {
    return field.dataset.errorEmail || 'Enter a valid email address.';
  }

  if (field.dataset.match && value) {
    const target = field.form?.querySelector(field.dataset.match);
    const targetValue = target?.value?.trim() || '';
    if (targetValue && value !== targetValue) {
      return field.dataset.errorMatch || 'Values must match.';
    }
  }

  if (field.validity.typeMismatch) {
    return field.dataset.errorType || field.dataset.errorEmail || 'Enter a valid value.';
  }

  if (field.validity.patternMismatch) {
    return field.dataset.errorPattern || 'Enter a valid value.';
  }

  if (field.validity.tooShort) {
    return field.dataset.errorMinlength || `Must be at least ${field.minLength} characters.`;
  }
...
function validateField(field) {
  if (!field || field.disabled || ['hidden', 'button', 'submit', 'reset'].includes(field.type)) {
    return true;
  }

  field.setCustomValidity('');
  const customError = getFieldErrorMessage(field);
  ...
  if (err) {
    if (isValid) {
      ...
      err.textContent = customError || field.validationMessage;
...
function validateDependentMatches(form, targetField) {
  const selector = `input[data-match="#${targetField.id}"], textarea[data-match="#${targetField.id}"]`;
  form.querySelectorAll(selector).forEach(validateField);
}
  ...
  document.querySelectorAll('form[data-validate]').forEach(form => {
    ...
    fields.forEach(field => {
      ...
      field.addEventListener(eventName, () => {
        validateField(field);
        if (field.id) validateDependentMatches(form, field);
      });
...
function initAutofillGuards() {
  document.querySelectorAll('input[data-autofill-guard="true"]').forEach(field => {
    const unlock = () => {
      field.removeAttribute('readonly');
      field.removeAttribute('aria-readonly');
    };

    ['focus', 'pointerdown', 'keydown', 'touchstart'].forEach(eventName => {
      field.addEventListener(eventName, unlock, { once: true });
    });
...
function initStatusBadge() {
  ...
  const map = { applied:'badge-applied', interview:'badge-interview', offer:'badge-offer', rejected:'badge-rejected', withdrawn:'badge-withdrawn', saved:'badge-saved' };
  ...
  statusSelect.addEventListener('change', () => {
    ...
    statusBadge.style.animation = 'badge-pop 0.3s cubic-bezier(0.34,1.56,0.64,1)';
  });
...
function initHeroParallax() {
  ...
  hero.addEventListener('mousemove', e => {
    ...
    if (dashboard) {
      const base = parseFloat(dashboard.style.transform?.match(/\+(.+?)\)/) || [0, 0])[1] || 0;
...
function initPasswordStrength(inputId = 'password') {
  ...
  input.addEventListener('input', () => {
    ...
    if (label) { label.textContent = lvl.l; label.style.color = lvl.c; }
  });
}

// Run if on register page
if (document.getElementById('password') && document.getElementById('pw-strength-bar')) {
...
function initCustomCursor() {
  ...
  const clickables = 'a, button, .btn, .feature-card, .bento-card, .document-card, .step-card, .stat-card, input, select, textarea, label';
