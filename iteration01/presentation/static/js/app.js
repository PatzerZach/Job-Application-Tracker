...
/* ── Theme ───────────────────────────────────────────────────── */
const THEME_KEY = 'apptrackr-theme';
...
function initConfirmDelete() {
  document.querySelectorAll('[data-confirm]').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      showConfirmModal(message, () => { if (form) form.submit(); });
    });
  });
}
  ...
  document.querySelectorAll('form[data-validate]').forEach(form => {
    form.addEventListener('submit', e => {
      ...
      if (!valid) {
        ...
        if (first) { first.style.animation = 'none'; first.offsetHeight; first.style.animation = 'shake 0.4s ease'; }
      }
    });
    ...
    form.querySelectorAll('[required]').forEach(field => {
      field.addEventListener('input', () => {
        if (field.value.trim()) {
          field.classList.remove('is-invalid');
          const err = field.closest('.form-group')?.querySelector('.form-error-text');
          if (err) err.style.display = 'none';
        }
      });
    });
